...
def motion_to_color(flow, maxmotion=None, verbose=False, original_color: bool = True):
    """
...
if __name__ == '__main__':
    ...
    flodir = 'D:\desktop\连续流场数据集\DNSplot'
    ...
    if isinstance(floname, list):
        for floi in floname:
            if os.path.isfile(floi):
                ...
                color_name=tmpname+'_color.png'
                colim=motion_to_color(vec_flow)
                plt.imshow(colim)
                #plt.show()
